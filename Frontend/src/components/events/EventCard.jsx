...
const EventCard = ({ event, onDelete }) => {
  ...
  const [liked, setLiked] = useState(event.likes.includes(user?._id));
  const [likeCount, setLikeCount] = useState(event.likes.length);
  const [commentText, setCommentText] = useState('');

  const handleLike = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/events/${event._id}/like`, {
        method: 'POST',
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      await fetch(`http://localhost:3000/api/events/${event._id}/comments`, {
        method: 'POST',
        headers: {
      ...
      setCommentText('');
      // Ideally, refetch event data here
    } catch (err) {
  ...
  return (
    <div className="card">
      <div className="flex items-center p-4">
        <img
          src={event.organizer?.profileImage || 'https://placehold.co/40x40?text=User'}
          alt="Organizer"
      ...
      <div className="p-4">
        ...
        <p className="text-sm text-gray-500 mt-2">Date: {formatDate(event.date)}</p>
        <p className="text-sm text-gray-500">Location: {event.location}</p>
        <p className="text-sm text-gray-500">RSVPs: {event.rsvps.length}</p>
        ...
        {isAdmin && (
          <div className="mt-4 flex space-x-2">
            <Link to={`/events/edit/${event._id}`} className="text-yellow-600 hover:underline">Edit</Link>
            <button onClick={() => onDelete(event._id)} className="text-red-600 hover:underline">Delete</button>
          </div>
        )}
      </div>
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="User Profile">
        <p><strong>Name:</strong> {event.organizer?.name}</p>
        <p><strong>Roll No:</strong> {event.organizer?.rollNo}</p>
        <p><strong>Email:</strong> {event.organizer?.email}</p>
        <button
          onClick={() => navigate(`/chats/${event.organizer?._id}`)}
